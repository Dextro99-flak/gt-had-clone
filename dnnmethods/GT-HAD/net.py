...
class BFBSelfAttention(nn.Module):
    ...
    def forward(self, x):
        B, H, W, C = x.shape
        assert C == self.dim, "BFB MHSA channel dimension mismatch."
        assert C % self.num_heads == 0, "BFB MHSA requires C % num_heads == 0."
        ...
        assert out.shape == (B, H, W, C), "BFB MHSA failed to restore feature shape."
        return out
...
class Attention(nn.Module):
    def __init__(self, dim, patch_size=3, patch_stride=3, attn_drop=0.,
        ...
        # AFB remains legacy GT-HAD attention. Only BFB is replaced with standard MHSA.
        self.bfb_num_heads = 4
        self.bfb_mhsa = BFBSelfAttention(
            dim=self.embed_dim, num_heads=self.bfb_num_heads, attn_drop=attn_drop, proj_drop=attn_drop
        )
    ...
    def _bfb_mhsa_forward(self, x):
        B, H, W, C = x.shape
        # BFB uses standard MHSA to improve background-context modeling while AFB
        # remains unchanged for anomaly suppression behavior.
        x = self.bfb_mhsa(x)
        assert x.shape == (B, H, W, C), "BFB MHSA path failed to restore shape."
        return x
    ...
    def forward(self, x, block_idx=0, match_vec=None):
        ...
        # Only BFB receives the MHSA replacement as a controlled modification.
        return out
